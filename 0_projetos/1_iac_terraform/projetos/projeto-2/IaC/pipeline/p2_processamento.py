...
def bamr_limpa_transforma_dados(spark, bucket, nome_bucket, ambiente_execucao_EMR):
	...
	# Grava no log
	bamr_grava_log("Log DSA - Importando os dados...", bucket)

	# Carrega o arquivo CSV
	reviews = spark.read.csv(path+'dataset.csv', header=True, escape="\"")

	# Grava no log
	bamr_grava_log("Log DSA - Dados Importados com Sucesso.", bucket)
	bamr_grava_log("Log DSA - Total de Registros: " + str(reviews.count()), bucket)
	bamr_grava_log("Log DSA - Verificando se Existem Dados Nulos.", bucket)

	# Calcula os valores ausentes
	null_columns_calc_list = bamr_calcula_valores_nulos(reviews)

	# Ação com base nos valores ausentes
	if (len(null_columns_calc_list) > 0):
		for column in null_columns_calc_list:
			bamr_grava_log("Coluna " + str(column[0]) + " possui " + str(column[2]) + " de dados nulos", bucket)
		reviews = reviews.dropna()
		bamr_grava_log("Dados nulos excluídos", bucket)
		bamr_grava_log("Log DSA - Total de Registros Depois da Limpeza: " + str(reviews.count()), bucket)
	else:
		bamr_grava_log("Log DSA - Valores Ausentes Nao Foram Detectados.", bucket)

	# Grava no log
	bamr_grava_log("Log DSA - Verificando o Balanceamento de Classes.", bucket)
	...
	# Grava no log
	bamr_grava_log("Log DSA - Existem " + str(count_positive_sentiment) + " reviews positivos e " + str(count_negative_sentiment) + " reviews negativos", bucket)

	# Cria o dataframe
	df = reviews

	# Grava no log
	bamr_grava_log("Log DSA - Transformando os Dados", bucket)
	...
	# Grava no log
	bamr_grava_log("Log DSA - Limpeza dos Dados", bucket)
	...
	# Grava no log
	bamr_grava_log("Log DSA - Os Dados de Texto Foram Limpos", bucket)
	bamr_grava_log("Log DSA - Tokenizando os Dados de Texto.", bucket)
	...
	# Grava no log
	bamr_grava_log("Log DSA - Removendo Stop Words.", bucket)
	...
	# Grava no log
	bamr_grava_log("Log DSA - Aplicando HashingTF.", bucket)
	...
	# Grava no log
	bamr_grava_log("Log DSA - Aplicando IDF.", bucket)
	...
	# Grava no log
	bamr_grava_log("Log DSA - Aplicando Word2Vec.", bucket)
	...
	# Grava no log
	bamr_grava_log("Log DSA - Padronizando os Dados com MinMaxScaler.", bucket)
	...
	# Grava no log
	bamr_grava_log("Log DSA - Salvando os Dados Limpos e Transformados.", bucket)
	...
	bamr_upload_dados_processados_bucket(HTFfeaturizedData, path + 'HTFfeaturizedData', s3_path + 'HTFfeaturizedData' , bucket, ambiente_execucao_EMR)
	bamr_upload_dados_processados_bucket(TFIDFfeaturizedData, path + 'TFIDFfeaturizedData', s3_path + 'TFIDFfeaturizedData', bucket, ambiente_execucao_EMR)
	bamr_upload_dados_processados_bucket(W2VfeaturizedData, path + 'W2VfeaturizedData', s3_path + 'W2VfeaturizedData', bucket, ambiente_execucao_EMR)
